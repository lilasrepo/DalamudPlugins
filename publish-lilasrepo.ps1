...
if (-not $SourceRoot -or -not (Test-Path -LiteralPath $SourceRoot)) {
  throw "SourceRoot '$SourceRoot' not set or not found. Pass -SourceRoot <path to your TC porting checkout> (or set `$env:TC_DALAMUD_ROOT)."
}
...
foreach ($p in $selected) {
  ...
  try {
    ...
    elseif ($DryRun) {
      ...
      # drop upstream CI: the forks are distribution mirrors, not build farms. Upstream workflows either
      # fail to validate (AutoDuty cntest.yml has an empty `on:` -> red X on every push) or run on our
      # release event against the INTERNATIONAL Dalamud (AutoDuty publish.yml -> 'Build & Publish to
      # Dynamis' fails with missing Dalamud refs and would publish to a channel that is not ours). Removing
      # the directory also drops the `workflow` token-scope requirement on pushes. (2026-09-02)
      Get-ChildItem -LiteralPath $work -Recurse -Force -Directory -Filter 'workflows' -ErrorAction SilentlyContinue |
        Where-Object { $_.Parent.Name -eq '.github' } | Remove-Item -Recurse -Force -ErrorAction SilentlyContinue
      Get-ChildItem -LiteralPath $work -Recurse -Force -Directory -ErrorAction SilentlyContinue |
...
if (-not $entries) { Write-Host "  (no entries to write)"; }
  ...
  if ($DryRun) {
    ...
    if ($PSCommandPath -and (Test-Path $PSCommandPath)) {
      ...
      # the user-facing README (install URL, how to read TC version numbers) lives next to this script too
      $readme = Join-Path (Split-Path -Parent $PSCommandPath) 'README.md'
      if (Test-Path $readme) { Copy-Item -Force $readme (Join-Path $mwork 'README.md') }
    }
